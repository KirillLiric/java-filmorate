...
CREATE TABLE IF NOT EXISTS feed (
    ...
    user_id BIGINT REFERENCES users(user_id) ON DELETE CASCADE,
    event_type VARCHAR(10) REFERENCES event(event_id) ON DELETE CASCADE,
    operation_type VARCHAR(10) REFERENCES operation(operation_id),
    time_stamp BIGINT
);

CREATE TABLE IF NOT EXISTS event (
    event_id INTEGER PRIMARY KEY,
    event_type VARCHAR(10) NOT NULL UNIQUE
)

CREATE TABLE IF NOT EXISTS operation (
    operation_id INTEGER PRIMARY KEY,
    operation_type VARCHAR(10) NOT NULL UNIQUE
)
